...
def test_work_order_valid_transitions(
...
def test_work_order_invalid_transitions_rejected(
    current: WorkOrderStatus, target: WorkOrderStatus
):
...
def test_work_order_scope_preserved():
    ...
    wo_with_scope = wo.model_copy(
        update={"scope": WorkOrderScope(included=["src/"], excluded=["tests/"])}
    )
